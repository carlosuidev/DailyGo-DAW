...
// Primero conectamos a la BBDD 
	...
	public static Riders getRiderS(String sEmailRider) {
		...
		ConexionBBDD miConexion = new ConexionBBDD();
		//
		String sBuscaProveedor = "SELECT * FROM riders" + " where MAIL_rid = '" + sEmailRider + "' ;";
		System.out.println("select que va a enviarse a conexion bbdd " + sBuscaProveedor);
		Riders rider = null;
		try {
			miConexion.conectar();
			...
			// Si hay resultado recuperamos los datos (como un FETCH de un CURSOR)
			while (rsResultado.next()) {
				System.out.println("Se crea bien el rider");
				// Creamos un objeto jugador por cada fila de la tabla (cadajugador)
				rider = new Riders(rsResultado.getString("DNI_RID"), rsResultado.getString("NOM_RID"), 
						rsResultado.getString("APE_RID"), String.valueOf(rsResultado.getInt("TLF_RID")), rsResultado.getString("MAIL_RID"), rsResultado.getString("PW_RID"), rsResultado.getString("IMG_RID"));
			}
		} catch (SQLException sqlex) {
			System.out.println("Error: " + sqlex.getMessage());
			sqlex.printStackTrace();
	...
	public static boolean editarPerfilRider(Riders riderEditado, Riders riderAeditar) {
		...
		// Primero conectamos a la BBDD
		ConexionBBDD miConexion = new ConexionBBDD();
		String updateUsuarios = "update riders set nom_rid='"+riderEditado.getNom_rid()
		+"', ape_rid='"+riderEditado.getApe_rid()+"', tlf_rid=" + Integer.valueOf(riderEditado.getTlf_rid())
		+", mail_rid='"+riderEditado.getMail_rid()
		+"', pw_rid='"+riderEditado.getPw_rid()+"' where mail_rid='"+riderAeditar.getMail_rid()+"';";
		try {
			miConexion.conectar();
			Integer iRes = miConexion.ejecutarUpdate(updateUsuarios);
			System.out.println("Resultado de insertar el cliente=" + iRes);
