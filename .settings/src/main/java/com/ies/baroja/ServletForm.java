...
public class ServletForm extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String borrarUsu = null;
		String salirUsu = null;
		String editarPerfilRider = null;
		editarPerfilRider = request.getParameter("nombreRE");
		salirUsu = request.getParameter("salirCuenta");
		borrarUsu = request.getParameter("borrarCuenta");
	...
	private static void loginUsuario(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (inicioSesCli != null) {
			...
			if (Controller.comprobarInicioSesionCliente(inicioSesCli, sPwd) == true && sesion.getAttribute("emailIU") == null) {
			...
			// && sesion.getAttribute("emailIU") == null
			if (Controller.comprobarInicioSesionProv(inicioSesProv, sPwd) == true && sesion.getAttribute("emailIP") == null) {
				// si coincide email y password y además no hay sesión iniciada
	...
	private static void borrarPerfilUsuario(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("LLega a borrar usuario");
		HttpSession misesion = request.getSession();
	 	Clientes cliente = (Clientes) misesion.getAttribute("emailIU");
		// en el caso de no ponga nombre la persona y solo le de a buscar salen todos los restaurantes que hay
	...
	private static void editarPerfilRider(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession misesion = request.getSession();
	 	Riders rideraEditar = (Riders) misesion.getAttribute("emailIR");
	 	System.out.println("Que es lo que llega aqui lol me rayo");
	 	System.out.println(misesion.getAttribute("emailIR"));
		Riders riderEditado = new Riders(null, request.getParameter("nombreRE"), 
				request.getParameter("apellidoRE"), request.getParameter("telRE"), 
				request.getParameter("correoRE"), request.getParameter("passwordRE"), null);
		// en el caso de no ponga nombre la persona y solo le de a buscar salen todos los restaurantes que hay
