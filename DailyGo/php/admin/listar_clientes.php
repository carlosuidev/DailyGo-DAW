...
try {
    ...
    if($buscar == ""){
        $datos = mysqli_query($conexion, "select * from clientes where MAIL_CLI <> 'admin@dailygo.com'");
    }else{
        $datos = mysqli_query($conexion, "select * from clientes where TLF_CLI LIKE '$buscar%' AND MAIL_CLI <> 'admin@dailygo.com'");
    }
